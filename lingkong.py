    ...
    def jineng(rl,j_hua):
        ...
        class weather():
            ...
            if "天气" in jn_hua:
                ...
                KEY = lk.config.weater_id_conf()  # API key
                UID = lk.config.weater_key_conf()  # 用户ID

                LOCATION = lk.config.city_conf()  # 所查询的位置，可以使用城市拼音、v3 ID、经纬度等
        ...
        def tuling(t_hua):
            ...
            city = lk.config.city_conf()#请自行在这里修改城市为自己的城市
            ...
            req={
                ...
                "userInfo": {
                    "apiKey": apikey,
                    "userId": apid #如果你改了apikey，请把这也改了
                }
            }
...
#model = sys.argv[1]
model = lk.config.snowboy_conf()#自己改唤醒模型路径
signal.signal(signal.SIGINT, signal_handler)
detector = snowboydecoder.HotwordDetector(model, sensitivity=0.7)
beg_name = lk.config.begin_conf()
tts(beg_name+"你好啊，欢迎使用灵空机器人，快说出唤醒词来唤醒我吧，啾咪")#可以改自己的名字上去，以后会放入配置页

...
# 运行环境
----
> * 1.使用者如是win平台，那么请去debian官网和virtualbox官网下载debian的镜像和virtualbox的安装包，然后进行安装、装载镜像
> * 2.如果你刚安装好debian，请执行
```shell
    sudo apt update
    sudo apt-get install git
```
安装git，然后自行确定你的debian有没有python3环境（正常来说是有的）


* 使用者如是Mac平台，那么可以直接开始安装了

* 使用者如果是debian平台，那么参考win平台的方法

* 使用者如是非debian的linux平台，那你可以暂时参考一下debian的安装方法，如有问题，欢迎通过下面的联系方式，联系作者
...
## 灵空个性化说明
#### 各位在使用的时候请勿擅自修改主文件，不然会引发不可避免的错误
#### 用户只需修改config.json中的东西即可（因为大部分要自己改的东西都集成到里面了）
## 后台
----
